...
def get_parser():
    """Define and parse command-line arguments for VidMotor Fine-tuning (PoseFormerV2)."""
    parser = argparse.ArgumentParser(description='VidMotor Fine-tuning (PoseFormerV2)')
...
class Processor():
    ...
    def load_optimizer(self):
        ...
        param_groups = [
            {"params": filter(lambda p: p.requires_grad, backbone_params), "lr": self.arg.lr_backbone},
            {"params": filter(lambda p: p.requires_grad, head_params), "lr": self.arg.lr_head},
            {"params": filter(lambda p: p.requires_grad, other_params), "lr": self.arg.lr_head}
        ]
    ...
    def start(self):
        ...
        for test_dataset_config in arg.test_feeder_args:
            ...
            self.save_path = f"{self.arg.work_dir}/{dataset.dataset_name}_inference-time"

...
from loss_utils import counterfactual_loss, non_causal_loss
import re
import warnings
...
def get_parser():
    ...
    parser.add_argument('--data-norm', default='minmax', help='normalization method for the skeleton sequence')
    ...
    parser.add_argument('--base-lr', type=float, default=0.0005, help='initial learning rate')
...
class Processor():
    ...
    def train(self, epoch, save_model=False):
        ...
        self.print_log(f'\tLearning rate: {self.optimizer_main.param_groups[0]["lr"]:.8f}.', self.save_path)
        self.print_log(f'\tMean training loss: main {np.mean(loss_value):.4f}; nc: {np.mean(loss_nc_value):.4f}.', self.save_path)
    ...
    def start(self):
        """Entry point: fine-tuning process including data split, training, and evaluation."""
        self.data_loaders = {"train": [], "test": []}
        try:
            dataset = Feeder(self.arg.train_feeder_args[0], self.arg.data_centered, self.arg.data_dim,
        ...
        dataset_subjects = sorted(set([re.sub(r'\d+', '', i.split('_')[0]) for i in dataset.sample_name]))
        
        # Select training subset size according to the given ratio
        random.seed(self.arg.data_seed)
        num_sub_train = int(self.arg.train_ratio * len(dataset_subjects))

        # Prepare output directory and logs
        self.save_path = f"{self.arg.work_dir}/{dataset.dataset_name}_training-data{self.arg.train_ratio}"
        os.makedirs(self.save_path, exist_ok=True)
        self.print_log(f'Parameters:\n{str(vars(self.arg))}\n', self.save_path)

        if num_sub_train == 0:
            self.print_log('No training subjects found.', self.save_path)
        else:
            random_count = 0
            while random_count < 1000:
                # Randomly select training subjects
                random.shuffle(dataset_subjects)
                train_subjects = dataset_subjects[:num_sub_train]

                # Match training sample names to training subject split
                train_sample_name = [sample for sample in dataset.sample_name if
                                     any(sub in sample for sub in train_subjects)]
                ...
                if len(set(train_set.label)) != self.arg.model_args['num_class']:
                    # self.print_log(
                    #     f"Training set has insufficient number of classes: {len(set(train_set.label))}, "
                    #     f"required: {self.arg.model_args['num_class']}",
                    #     self.save_path
                    # )
                    random_count += 1
                    continue
                    ...
                    self.print_log(
                        f"Training set contains all classes: {len(set(train_set.label))}",
                        self.save_path
                    )

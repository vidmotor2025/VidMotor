...
class Feeder(Dataset):
    ...
    def __init__(self, dataset_info, data_centered, data_dim, data_norm, score_norm, label_type, data_length=64,
                 data_indices=None, stage=None, require_index_t=False, require_mask=False):
        ...
        self.require_index_t = require_index_t
        self.require_mask = require_mask
        if self.partition:
            self.right_arm = np.array([13])
            self.left_arm = np.array([16])
            self.right_leg = np.array([4, 5, 6])
            self.left_leg = np.array([1, 2, 3])
            self.h_torso = np.array([11, 14, 12, 15])
            self.w_torso = np.array([7, 9, 0, 10])
            self.new_idx = np.concatenate(
                (self.right_arm, self.left_arm, self.right_leg, self.left_leg, self.h_torso, self.w_torso), axis=-1)
            # except for joint no.21 (8 for human36m)
        # Load raw data
        self.load_data()
        if self.require_index_t:
        ...
        if self.stage is not None:
            self.unify_time_length_and_rotate()
    ...
    def unify_time_length_and_rotate(self):
        ...
        for sample_index in range(len(self.data)):
            ...
            if self.stage == 'train':
                ...
                poses = self.rand_view_transform(poses, agx, agy, s)
                if T * 100 >= self.data_length:
                    random_idx = random.sample(list(np.arange(T)) * 100, self.data_length)
                else:
                    random_idx = np.random.choice(T, self.data_length, replace=True).tolist()
                random_idx.sort()
                data = np.zeros((self.data_length, V, C))
                data[:, :, :] = poses[random_idx, :, :]
                index_t = 2 * np.array(random_idx).astype(np.float32) / self.data_length - 1
                ...
                s = 1.0
                poses = self.rand_view_transform(poses, agx, agy, s)
                idx = np.linspace(0, T - 1, self.data_length).astype(int)
                data = np.zeros((self.data_length, V, C))
                data[:, :, :] = poses[idx, :, :]
                index_t = 2 * idx.astype(np.float32) / self.data_length - 1
...
class PretrainFeeder(Feeder):
    def __init__(self, dataset_info, data_centered, data_dim, data_norm, score_norm, label_type, data_length, stage='train', require_index_t=True):
        """
        ...
        super().__init__(dataset_info, data_centered, data_dim, data_norm, score_norm, label_type, data_length, stage='train', require_index_t=True)
    ...
    def __getitem__(self, index):
        ...
        if self.dataset_name == 'AGF-Olympics':
            if self.require_index_t:
                index_t = self.index_list[index]
                return sample_name, data, label, index_t, index
            else:
                return sample_name, data, label, index
        elif self.dataset_name in ['Rhythmic-Gymnastics', 'Push-Up']:
            class_label = self.class_label[index]
            if self.require_index_t:
                index_t = self.index_list[index]
                return sample_name, data, label, class_label, index_t, index
            else:
                return sample_name, data, label, class_label, index
        else:
            subject_name = self.subject_name[index]
            class_label = self.class_label[index]
            if self.require_index_t:
                index_t = self.index_list[index]
                return subject_name, sample_name, data, label, class_label, index_t, index
            else:
                return subject_name, sample_name, data, label, class_label, index
...
def get_parser():
    """Define and parse command-line arguments for VidMotor Fine-tuning (CTR-GCN)."""
    parser = argparse.ArgumentParser(description='VidMotor Fine-tuning (CTR-GCN)')
...
class Processor():
    ...
    def load_model(self, ce_weights=None):
        ...
        if self.arg.model_pretrain != 'None':
            ...
            for k, v in model_pretrain.items():
                ...
                if new_k in model_dict and 'fc' not in new_k:
    ...
    def eval(self):
        ...
        for loader_index in range(4):
            ...
            self.print_log(f"Average inference time for center-{loader_index + 1}: {avg_time} milliseconds",
                           self.save_path)

...
def get_parser():
    """Define and parse command-line arguments for VidMotor Pretraining (SkateFormer)."""
    parser = argparse.ArgumentParser(description='VidMotor Pretraining (SkateFormer)')
    ...
    parser.add_argument('--base-lr', type=float, default=0.00002, help='initial learning rate')
    ...
    parser.add_argument('--num-epoch', type=int, default=1000, help='number of epochs to train')
...
class Processor():
    ...
    def pretrain(self, epoch, save_model=False):
        ...
        for train_loader in self.data_loaders['train']:
            ...
            with tqdm(train_loader, ncols=100) as t:
                ...
                if train_loader.dataset.dataset_name == 'AGF-Olympics':
                    ...
                    for batch_idx, (sample_name, data, label, index_t, index) in enumerate(t):
                        data = Variable(data.float().cuda(self.output_device), requires_grad=False)
                        index_t = Variable(index_t.float().cuda(self.output_device), requires_grad=False)
                        output, feats = self.model(data, index_t)
                    ...
                    for batch_idx, (sample_name, data, label, class_label, index_t, index) in enumerate(t):
                        data = Variable(data.float().cuda(self.output_device), requires_grad=False)
                        index_t = Variable(index_t.float().cuda(self.output_device), requires_grad=False)
                        output, feats = self.model(data, index_t)
                        label = Variable(label.float().cuda(self.output_device), requires_grad=False)
                        ...
                        if self.arg.contrastive_weight != 0:
                            loss_contrastive = cross_individual_contrastive_loss(feats, class_label,
                    ...
                    for batch_idx, (subject_name, sample_name, data, label, class_label, index_t, index) in enumerate(t):
                        data = Variable(data.float().cuda(self.output_device), requires_grad=False)
                        index_t = Variable(index_t.float().cuda(self.output_device), requires_grad=False)
                        output, feats = self.model(data, index_t)
                        label = Variable(label.float().cuda(self.output_device), requires_grad=False)
        ...
        self.print_log(f'\tLearning rate: {self.optimizer.param_groups[0]["lr"]:.8f}.')
        self.print_log(
        ...
        for train_dataset_config in arg.train_feeder_args:
            try:
                dataset = Feeder(train_dataset_config, self.arg.data_centered, self.arg.data_dim, self.arg.data_norm,
                                 self.arg.score_norm, self.arg.label_type, data_length=None, stage='train',

...
def get_parser():
    """Define and parse command-line arguments for VidMotor Pretraining (CTR-GCN)."""
    parser = argparse.ArgumentParser(description='VidMotor Pretraining (CTR-GCN)')
    ...
    parser.add_argument('--base-lr', type=float, default=0.005, help='initial learning rate')
...
class Processor():
    """Main pretraining controller that manages model, optimizer, and pretraining process."""
    ...
    def pretrain(self, epoch, save_model=False):
        """Main pretraining loop for one epoch."""
        self.model.train()
    ...
    def start(self):
        """Entry point: pretraining process."""

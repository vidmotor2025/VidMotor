...
class Processor():
        ...
        with torch.no_grad():
            for test_loader in self.data_loaders['test']:
                with tqdm(test_loader, ncols=100) as t:
                    ...
                    self.print_log(
                        f'\tTest Dataset: {test_loader.dataset.dataset_name}; Accuracy: {accuracy * 100:.2f}',
                        self.save_path)
    ...
    def start(self):
        ...
        try:
            train_set = Feeder(self.arg.train_feeder_args[0], data_centered=self.arg.data_centered,
                               data_dim=self.arg.data_dim, data_norm=self.arg.data_norm, score_norm=self.arg.score_norm,
                               label_type=self.arg.label_type, stage='train', num_clips=1)

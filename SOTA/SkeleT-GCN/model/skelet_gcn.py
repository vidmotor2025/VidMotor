...
from .utils.tcn import mstcn, MSTCN, dgmstcn
from .utils.gcn import residual_block, grouped_mapping_framework
from .utils.init_mapping_matrices import mapping_matrices
from einops import rearrange
import numpy as np
from .utils.heads.simple_head import GCNHead
...
class SkeleT_GCN(nn.Module):
    ...
    def __init__(self,
        ...
        if with_instance_pooling:
            self.instance_pooling_block = InstancePoolingModule(in_channels, base_channels, self.stage_joints[0],
                                                                **ip_kwargs)
        ...
        if self.in_channels != self.base_channels:
            modules = [STBlock(base_channels if with_instance_pooling else in_channels, base_channels,
                               ...
                               self.graph_tools.get_M(self.stage_joints[0],
                                                      self.stage_joints[0]) if with_instance_pooling else None,
                               M_adaptive if with_instance_pooling else None,
                               t_sample=None, residual=True if with_instance_pooling else False,
                               same_transformation_importance=same_transformation_importance,
                               split_way=split_way,
        ...
        for i in range(2, num_stages + 1):
            ...
            if stride == 1:
                modules.append(STBlock(in_channels, out_channels,
                                       ...
                                       self.graph_tools.get_M(self.stage_joints[i - 2], self.stage_joints[i - 1]),
                                       M_adaptive,
                                       t_sample=None, t_factor=stride, residual=True,
                                       same_transformation_importance=same_transformation_importance,
                                       split_way=split_way,
                                       concat_way=concat_way,
                                       **lw_kwargs[i - 1]))
                ...
                modules.append(STBlock(in_channels, out_channels,
                                       ...
                                       self.graph_tools.get_M(self.stage_joints[i - 2], self.stage_joints[i - 1]),
                                       M_adaptive,
                                       t_sample='down_sample', t_factor=stride, residual=True,
                                       same_transformation_importance=same_transformation_importance,
                                       split_way=split_way,
                                       concat_way=concat_way,
                                       **lw_kwargs[i - 1]))
        ...
        self.cls_head = GCNHead(num_classes=num_class, in_channels=256)

    def forward(self, x):
        N, C, T, V, I = x.size()
        # N, I, T, V, C = x.size()
        # x = x.permute(0, 1, 3, 4, 2).contiguous()
        x = x.permute(0, 4, 3, 1, 2).contiguous()  # N,I,V,C,T
        ...
        if hasattr(self, 'instance_pooling_block'):
            ...
            x = x.reshape((N, I) + x.shape[1:])  # N ,I, C, T, V

        # classification
        cls_score = self.cls_head(x)  # N,C

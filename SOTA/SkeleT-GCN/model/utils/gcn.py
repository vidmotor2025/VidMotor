    ...
    def __init__(self,
        ...
        if self.split_way == 'conv+split':
            if self.num_groups != 1:
                self.conv_pre = nn.Sequential(
                    ...
                    nn.BatchNorm2d(self.inner_inc * self.num_groups),
        ...
        for i in range(self.num_groups):
            if gcn_type == 'unit_gcn':
                ...
                basic_gcn_blocks.append(
                    unit_ctrgcn(self.inner_inc, self.inner_outc, A[i], with_res=False, **gcn_kwargs))
            else:
                raise ValueError
    ...
    def __init__(self,
        ...
        if in_channels != out_channels or t_sample is not None:
            ...
            self.down = nn.Sequential(
                nn.Conv2d(in_channels, out_channels, kernel_size=(kernel_size, 1), stride=(t_factor, 1),
                          padding=(pad, 0)),

...
def import_class(name):
    """Works for both direct and reproducibility runs"""
    if not name.startswith(('SOTA.', 'VidMotor.')):
        # Try both with and without prefix
        for prefix in ['', 'SOTA.HD-GCN.']:
            try:
...
class AHA(nn.Module):
    def __init__(self, in_channels, num_layers, CoM):
        ...
        groups = get_groups(dataset='Human36m', CoM=CoM)
...
class HD_Gconv(nn.Module):
    def __init__(self, in_channels, out_channels, A, adaptive=True, residual=True, att=False, CoM=9):
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_joints=25, num_person=1, graph=None, graph_args=dict(), in_channels=3,
        ...
        # self.dataset = 'NTU' if num_joints == 25 else 'UCLA'

...
def get_parser():
    """Define and parse command-line arguments for HD-GCN."""
    parser = argparse.ArgumentParser(description='HD-GCN')
...
class Processor():
    ...
    def adjust_learning_rate(self, epoch, idx, length_trainloader):
        """Adjust the learning rate."""
        if self.arg.optimizer == 'SGD' or self.arg.optimizer == 'Adam':
            if epoch < self.arg.warm_up_epoch:
                lr = self.arg.base_lr * (epoch + 1) / self.arg.warm_up_epoch
            else:
                T_max = length_trainloader * (self.arg.num_epoch - self.arg.warm_up_epoch)
                T_cur = length_trainloader * (epoch - self.arg.warm_up_epoch) + idx
        ...
        for train_loader in self.data_loaders['train']:
            ...
            with tqdm(train_loader, ncols=100) as t:
                for batch_idx, (data, label, index) in enumerate(t):
                    self.adjust_learning_rate(epoch, batch_idx, len(train_loader))

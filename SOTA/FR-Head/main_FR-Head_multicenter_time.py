...
def get_parser():
    """Define and parse command-line arguments for FR-Head."""
    parser = argparse.ArgumentParser(description='FR-Head')
    ...
    parser.add_argument('--cl-mode', choices=['ST-Multi-Level'], default=None, help='mode of Contrastive Learning Loss')
    parser.add_argument('--cl-version', choices=['V0', 'V1', 'V2', "NO FN", "NO FP", "NO FN & FP"], default='V0', help='different way to calculate the cl loss')
    ...
    parser.add_argument('--pred-threshold', type=float, default=0.0, help='threshold to define the confident sample')
    ...
    parser.add_argument('--step', type=int, default=[16, 25], nargs='+', help='the epoch where optimizer reduce the learning rate')
    parser.add_argument('--lr-decay-rate', type=float, default=0.1, help='decay rate for learning rate')
    parser.add_argument('--warm-up-epoch', type=int, default=2)
...
class Processor():
                ...
                for batch_idx, (data, label, index) in enumerate(t):
                    ...
                    if self.arg.cl_mode is not None:
                        if epoch > self.arg.start_cl_epoch:
                            loss += self.arg.w_cl_loss * cl_loss.mean()
    ...
    def eval(self):
        ...
        for loader_index in range(4):
            ...
            self.print_log(f"Average inference time for center-{loader_index + 1}: {avg_time} milliseconds", self.save_path)

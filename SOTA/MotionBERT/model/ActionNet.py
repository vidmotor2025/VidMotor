...
class ActionNet(nn.Module):
    def __init__(self, backbone, dim_rep=512, num_class=60, dropout_ratio=0., version='class', hidden_dim=2048,
                 num_joints=17):
        super(ActionNet, self).__init__()
        self.backbone = backbone
        self.feat_J = num_joints
        if version == 'class':
            self.head = ActionHeadClassification(dropout_ratio=dropout_ratio, dim_rep=dim_rep, num_class=num_class,

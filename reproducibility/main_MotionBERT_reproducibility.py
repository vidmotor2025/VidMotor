...
from SOTA.MotionBERT.model.tools import *
...
def get_parser():
    ...
    parser.add_argument('--lr-head', type=float, default=0.01, help='initial learning rate for model')
        ...
        with torch.no_grad():
            for test_loader in self.data_loaders['test']:
                ...
                if epoch == self.arg.num_epoch - 1:
                    ...
                    writer.write_result(
                        dataset=test_loader.dataset.dataset_name, model_name='MotionBERT', accuracy=final_accuracy * 100, f1=final_f1 * 100,
...
def update_arg_from_params(arg, dataset_name, idx):
    ...
    mapping = {
        ...
        'lr': {'lr_backbone': 1.0, 'lr_head': 10.0},
        'ds': ['data_seed']
    }
    for k, v in mapping.items():
        if isinstance(v, dict):
            for key_name, ratio in v.items():
                setattr(arg, key_name, p[k] * ratio)
        else:
            for key_name in v:
                if k == 'bs':
...
def import_class(name, base_module='SOTA.MotionBERT'):

...
def get_parser():
    """Define and parse command-line arguments for SkeleT-GCN."""
    parser = argparse.ArgumentParser(description='SkeleT-GCN')
        ...
        with torch.no_grad():
            for test_loader in self.data_loaders['test']:
                ...
                for batch_idx, (data, _, index) in enumerate(test_loader):
                    ...
                    output = self.model(data)  # (bs*nc,class)
                    output = output.reshape(bs, nc, output.shape[-1])  # (bs,nc,class)
                ...
                if epoch == self.arg.num_epoch - 1:
                    final_accuracy, final_balanced_acc, final_f1, final_auc = self.metrics['accuracy'], self.metrics[
                        'balanced_acc'], self.metrics['f1'], self.metrics['auc']
                    os.makedirs(os.path.dirname(self.arg.save_path), exist_ok=True)
                    writer = ResultWriter(self.arg.save_path)
                    writer.write_result(
                        dataset=test_loader.dataset.dataset_name, model_name='SkeleT-GCN', accuracy=final_accuracy * 100,
                        f1=final_f1 * 100, auc=final_auc * 100, balanced_acc=final_balanced_acc * 100)
                    row = (
    ...
    def start(self):
        ...
        for test_dataset_config in arg.test_feeder_args:
            ...
            if os.path.exists(os.path.join(
                    os.path.join(self.arg.save_path.split('.xlsx')[0], test_set.dataset_name,
                                 'lr' + str(self.arg.base_lr) + '_dataseed' + str(self.arg.data_seed))
            )):
                print(f'\t\t\tSkip existing result: {self.arg.data_seed}')
                continue
...
def import_class(name, base_module='SOTA.SkeleT-GCN'):

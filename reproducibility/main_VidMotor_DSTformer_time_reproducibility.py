...
class Processor():
    ...
    def load_optimizer(self):
        ...
        # self.scheduler = StepLR(self.optimizer_main, step_size=1, gamma=self.arg.lr_decay)
        # self.scheduler = StepLR(self.optimizer_nc, step_size=1, gamma=self.arg.lr_decay)
...
def update_arg_from_params(arg, dataset_name, idx):
    ...
    mapping = {
        'bs': ['train_batch_size'],
        'DSTformer_backbone_lr': ['lr_backbone', 'lr_head'],
        'ds': ['data_seed']
    }
    for k, v in mapping.items():
        for key_name in v:
            if k in ['DSTformer_backbone_lr', 'bs']:

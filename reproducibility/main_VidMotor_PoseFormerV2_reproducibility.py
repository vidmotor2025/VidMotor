...
def get_parser():
    ...
    parser.add_argument('--lr-backbone', type=float, default=0.001, help='initial learning rate for backbone model')
    parser.add_argument('--lr-head', type=float, default=0.001, help='initial learning rate for model')
        ...
        with torch.no_grad():
            for test_loader in self.data_loaders['test']:
                ...
                if epoch == self.arg.num_epoch - 1:
                    ...
                    writer.write_result(
                        dataset=test_loader.dataset.dataset_name, model_name='VidMotor_PoseFormerV2', accuracy=final_accuracy * 100, f1=final_f1 * 100,
...
def update_arg_from_params(arg, dataset_name, idx):
    ...
    mapping = {
        ...
        'PoseFormerV2_backbone_lr': ['lr_backbone', 'lr_head'],
        'ds': ['data_seed']
    }
    for k, v in mapping.items():
        for key_name in v:
            if k in ['PoseFormerV2_backbone_lr', 'bs']:

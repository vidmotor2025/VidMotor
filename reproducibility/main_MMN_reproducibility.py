...
from SOTA.MMN.loss_utils import LabelSmoothingCrossEntropy
...
def get_parser():
    ...
    # parser.add_argument('--num-epoch', type=int, default=120, help='number of epochs to train')
    ...
    def eval(self,epoch):
        ...
        with torch.no_grad():
            for test_loader in self.data_loaders['test']:
                ...
                n_classes = self.arg.model_args['num_classes']
                if n_classes == 2:
                ...
                if epoch == self.arg.num_epoch - 1:
                    ...
                    writer.write_result(
                        dataset=test_loader.dataset.dataset_name, model_name='MMN',
...
def import_class(name, base_module='SOTA.MMN'):

...
def get_parser():
    """Define and parse command-line arguments for VidMotor finetuning (CTR-GCN)."""
        ...
        with torch.no_grad():
            for test_loader in self.data_loaders['test']:
                ...
                if epoch == self.arg.num_epoch - 1:
                    ...
                    writer.write_result(
                        dataset=test_loader.dataset.dataset_name, model_name='VidMotor_CTR-GCN',
                        accuracy=final_accuracy * 100, f1=final_f1 * 100,
...
def update_arg_from_params(arg, dataset_name, idx):
    ...
    mapping = {
        ...
        'CTR-GCN_backbone_lr': ['base_lr'],
        'ds': ['data_seed']
    }
    for k, v in mapping.items():
        for key_name in v:
            if k in ['CTR-GCN_backbone_lr', 'bs']:
